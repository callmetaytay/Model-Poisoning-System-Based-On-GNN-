...
def kl_divergence(P, Q):
    ...
    P = P + [epsilon for _ in range(len(P))]
    Q = Q + [epsilon for _ in range(len(Q))]
...
def load_data(datapath_str, dataset_str):
    ...
    elif dataset_str in ["acm", "amazon_photo", "amazon_cs", "citeseer_full", "coauthor_phy", "dblp", "flickr", "blogcatalog", "uai"]:
        return load_graphgallery_data(datapath_str, dataset_str)
    else:
...
def load_data_original(datapath_str, dataset_str):
    ...
    for i in range(len(y_test)):
        label.append(y_test[i].tolist().index(max(y_test[i])))
...
def set_up_seed(seed):
    """
    设置随机种子以确保实验可重复，支持 PyTorch 和 DGL。
    :param seed: 随机种子
    """
    random.seed(seed)  # 设置 Python 标准库的随机种子
    np.random.seed(seed)  # 设置 NumPy 的随机种子
    torch.manual_seed(seed)  # 设置 PyTorch 的随机种子
    torch.cuda.manual_seed_all(seed)  # 设置所有 GPU 的随机种子（如果有多个GPU）

    # 设置 DGL 的随机种子
    dgl.random.seed(seed)

    # 如果你使用的是 cudnn 后端（适用于卷积层），为了保证可重复性，我们做一些额外的设置：
    torch.backends.cudnn.deterministic = True  # 保证每次计算结果一致
    torch.backends.cudnn.benchmark = False  # 对某些特定输入进行优化，关闭此项保证重复性
