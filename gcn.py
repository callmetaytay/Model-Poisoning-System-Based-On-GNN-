...
class GCN(nn.Module):
    ...
    def inference(self, g, x, device):
        ...
        for l, layer in enumerate(self.layers):
            ...
            num_workers = 0 if device.type == 'cuda' else self.num_workers
            dataloader = dgl.dataloading.DataLoader(
                g,
                th.arange(g.number_of_nodes(), device=device, dtype=th.int32),
                sampler,
            ...
            for input_nodes, output_nodes, blocks in dataloader:
                ...
                h = x[input_nodes.to(device)].to(device)
                h = layer(block, h)
                if l != len(self.layers) - 1:
                    h = self.activation(h)
                    h = self.dropout(h)

                y[output_nodes.to(device)] = h
    ...
    def extract_embedding(self, g, x, device):
        for l, layer in enumerate(self.layers):
            ...
            dataloader = dgl.dataloading.DataLoader(
                g,
                th.arange(g.number_of_nodes(), dtype=th.int32),
                sampler,
                batch_size=self.batch_size,
                shuffle=True,
                drop_last=False,
